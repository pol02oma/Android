...
class HtmlTreeBuilder extends TreeBuilder
{
  ...
  private boolean inSpecificScope(String[] paramArrayOfString1, String[] paramArrayOfString2, String[] paramArrayOfString3)
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      {
        return i;
        bool = StringUtil.in(str, paramArrayOfString2);
        i = 0;
      }
      while (bool);
  ...
  {
    if ((paramStartTag.isSelfClosing()) && (!Tag.isKnownTag(paramStartTag.name())))
    {
      Element localElement2 = insertEmpty(paramStartTag);
      process(new Token.EndTag(localElement2.tagName()));
  ...
  List<Node> parseFragment(String paramString1, Element paramElement, String paramString2, ParseErrorList paramParseErrorList)
  {
    ...
    while (true)
    {
      localElement = new Element(Tag.valueOf("html"), paramString2);
      this.doc.appendChild(localElement);
      this.stack.push(localElement);
      resetInsertionMode();
      runParser();
      if (paramElement == null)
        break;
      return localElement.childNodes();
      label139: if (StringUtil.in(str, new String[] { "iframe", "noembed", "noframes", "style", "xmp" }))
      {
