  ...
  implements ConnPool<T, E>, ConnPoolControl<T>
{
  ...
  private RouteSpecificPool<T, C, E> getPool(T paramT)
  {
    ...
    if (localObject == null)
    {
      localObject = new RouteSpecificPool(paramT, paramT)
      {
        protected E createEntry(C paramC)
        {
          return AbstractConnPool.this.createEntry(this.val$route, paramC);
        }
      };
    ...
    throws IOException, InterruptedException, TimeoutException
  {
    ...
    while (true)
    {
      ...
      for (int n = 0; ; n++)
      {
        PoolEntry localPoolEntry3;
        if (n < j)
        {
          localPoolEntry3 = localRouteSpecificPool.getLastUsed();
      ...
      try
      {
        ...
        if ((bool2) || (localDate == null) || (localDate.getTime() > System.currentTimeMillis()))
          continue;
        label461: throw new TimeoutException("Timeout waiting for connection");
      }
      finally
      {
        localRouteSpecificPool.unqueue(paramPoolEntryFuture);
  ...
  public Future<E> lease(T paramT, Object paramObject, FutureCallback<E> paramFutureCallback)
  {
    ...
    if (this.isShutDown)
      throw new IllegalStateException("Connection pool shut down");
    return new PoolEntryFuture(this.lock, paramFutureCallback, paramT, paramObject)
    {
      public E getPoolEntry(long paramLong, TimeUnit paramTimeUnit)
        throws InterruptedException, TimeoutException, IOException
      {
        return AbstractConnPool.this.getPoolEntryBlocking(this.val$route, this.val$state, paramLong, paramTimeUnit, this);
      }
    };
  }
    ...
    throws IOException
  {
    ...
    Iterator localIterator2 = this.leased.iterator();
